...
dim integer playermapx
dim integer playermapy
...
DIM integer player.x=32*14
DIM integer player.y=32*10
...
dim integer player.leftanims(4)=(13,14,15,16,17)
dim integer player.leftanimvalue=0
dim integer player.rightanims(4)=(1,2,3,4,5)
...
data 35,35,35,35,35,35,35,35,35,17,35,17,35,35,35,35,35,35,35,35
...
for x = 1 to 15     ' 15 blocks high
  ...
  '  blit z,y,32*tt+startx,32*xx+starty,32,32,1
...
'playerindex = MAPData(playermapy,playermax)
'playerindex2 = MAPold(playermapx + ((playermapy * MAPWIDTH)))
'checkcollision
' bottom left corner of the map is 15,1
newcollision
checkjump
'if playerindex2 <> 35 then player.vely=0
'print @(0,400),playermapy,playermapx,playerindex,MAPData(playermapy,playermapx),playerindex2
print @(0,400),playerindex2,player.x,player.y
...
if keya$ = " 131" then   ' moving right
  ...
  player.rightanimvalue = player.rightanimvalue +1
    if player.rightanimvalue =5 then player.rightanimvalue =0
end if
...
if keya$ = " 130" then      ' moving left
  ...
  player.leftanimvalue = player.leftanimvalue +1
    if player.leftanimvalue =5 then player.leftanimvalue =0
...
'dim integer rpixel=0
   ...
   bpixel = pixel (player.x+10,player.y+32)    ' read below
     pixel player.x+10,player.y+32  
    rpixel = pixel (player.x+23,player.y+16)    ' in front facing right
    pixel player.x+23,player.y+16
'         pixel player.x+10,player.y+3     'above
   lpixel = pixel (player.x,player.y+16)      ' behind
      pixel player.x,player.y+16

        if bpixel <> 0 then ' we hit a tile
        player.vely = 0
        player.lockjump=0
        player.jumping=0
        end if
      
       if lpixel <> 0 then ' we hit a tile
'          player.velx = velmoving
      ...
      if rpixel <> 0 then ' we hit a tile
        player.velx = 0
        player.lockjump=0
        player.jumping=0
        end if
  ...
  if player.faceright=1 then
    ...
    playerindex = MAPold(playermapx +(playermapy*MAPWIDTH))
       if playerindex =0 then
            playerx=(playermapx-1) *TILEWIDTH
            player.velx=0
        end if
  else
